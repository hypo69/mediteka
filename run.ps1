<#
.SYNOPSIS
    Главный лончер проекта Mediteka. Запускает FastAPI-сервер и сопутствующие сервисы.

.DESCRIPTION
    Активирует виртуальное окружение, загружает конфигурацию из config.json,
    проверяет зависимости и запускает локальный сервер вместе с
    сопутствующими сервисами (Cloudflare Tunnel, Foundry), если они включены.
