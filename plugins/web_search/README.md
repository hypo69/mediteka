...
4. **💻 `gemini_cli` (Google Gemini CLI)** — поиск через локальный терминальный агент Google Gemini CLI с динамическим выбором модели (`gemini_cli_searcher.py`).
5. **🚀 `agy` (Antigravity AGY)** — агентный поиск через встроенные инструменты `google.antigravity` (`BuiltinTools.SEARCH_WEB`, `READ_URL_CONTENT`) (`agy_searcher.py`).
...
- `gemini_cli_searcher.py`: модуль поиска Google Gemini CLI `GeminiCliWebSearcher`.
