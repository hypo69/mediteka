...
def _split_records(records: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
    ...
    series = sorted([r for r in records if (r.get('media_type') or r.get('type')) == 'series'], key=key)
    movies = sorted([r for r in records if (r.get('media_type') or r.get('type')) == 'movie'], key=key)
