...
def search_media(
    ...
    if api_key:
        try:
            ...
            logger.error(f"Ошибка FAISS поиска: {e_faiss}")
            results_list = []
    else:
        logger.warning("GEMINI_API_KEY не найден для семантического RAG-поиска.")
    ...
    try:
        ...
        for item in results_list:
            ...
            # Фильтр по дискам
            path = meta.get('path', '')
            is_web_url = path.startswith(('http://', 'https://'))
            if path and not is_web_url and active_paths and not any(path.upper().startswith(p.upper()) for p in active_paths):
                continue

            if category and meta.get('main_category') != category:
                continue
            if media_type and (meta.get('media_type') or meta.get('type')) != media_type:
                continue
            ...
            'results': [
                {
                    ...
                    'media_type': r['meta'].get('media_type') or r['meta'].get('type', ''),
                    'type': r['meta'].get('media_type') or r['meta'].get('type', ''),
...
def get_media_card(disk_name: str, title: str, media_type: str = '') -> str:
    ...
    try:
        ...
        record = db.get_media(disk_name, title) if disk_name else {}
        if not record:
            record = db.find_any_disk(title)
        ...
        resolved_type = record.get('media_type') or media_type or 'movie'
        ...
        if resolved_type == 'series':
            ...
            # Загружаем краткую сводку по сезонам из базы
            series_id = record.get('id', 0)
            if series_id:
                try:
                    import sqlite3
                    with sqlite3.connect(_DB_FILE) as conn:
                        ...
                        seasons_rows = conn.execute(
                            "SELECT title, plot, final_verdict FROM media WHERE parent_id = ? AND media_type = 'season' ORDER BY id ASC",
                            (series_id,)
                        ).fetchall()
                        if seasons_rows:
                            response['seasons_summary'] = [
                                {
                                    'title': s['title'],
                                    'plot': s['plot'] or '',
                                    'final_verdict': s['final_verdict'] or ''
                                }
                                for s in seasons_rows
                            ]
                except Exception as ex_s:
                    logger.warning(f"Не удалось загрузить сезоны для сериала {title}: {ex_s}")
...
def find_by_exact_title(title: str, media_type: str = '') -> str:
    ...
    try:
        ...
        for record in all_records:
            rec_type = record.get('media_type', '')
            # По умолчанию исключаем фрагменты отдельных сезонов и серий, если явно не запрошены
            if not media_type and rec_type in ('season', 'episode'):
                continue
            if media_type and rec_type != media_type:
                continue
...
def get_random_media(
    ...
    try:
        ...
        if media_type:
            filtered = [r for r in filtered if (r.get('media_type') or r.get('type')) == media_type]
        if mood:
        ...
        if not filtered:
            return json.dumps({
                ...
                'filters': {'category': category, 'media_type': media_type, 'mood': mood},
...
# =============================================================================
# Инструмент воспроизведения медиа (play_media)
# =============================================================================

_play_media_schema = {
    'name': 'play_media',
    'description': (
        'Запускает воспроизведение фильма, сериала или видео на домашнем плеере kino.davidka.net. '
        'Используй этот инструмент ВСЕГДА когда пользователь просит включить, запустить, '
        'воспроизвести, поставить, показать какой-либо фильм или сериал. '
        'Источниками могут быть как файлы на диске, так и онлайн-ссылки (YouTube и др.).'
    ),
    'parameters': {
        'type': types.Type.OBJECT,
        'properties': {
            'title': {
                'type': types.Type.STRING,
                'description': 'Название фильма или сериала для воспроизведения (на русском или оригинальном языке)',
            },
        },
        'required': ['title'],
    },
...
def get_player_tools() -> list:
    """Возвращает список инструментов для управления плеером.

    Returns:
        list: Список схем инструментов в формате Gemini Function Calling.
    """
    return [
        {
            'function_declarations': [_play_media_schema],
        },
    ]


def make_play_dispatcher(room_id: str, db_path=None):
    """Фабрика диспетчера инструмента play_media.

    Args:
        room_id: Идентификатор WebSocket-комнаты плеера.
        db_path: Путь к media.db (опционально, по умолчанию берётся из MEDIA_DB).

    Returns:
        Callable: Синхронная функция-диспетчер для передачи в ask_with_tools_stream.
    """
    import asyncio

    _db_path = db_path or _DB_FILE

    def dispatcher(func_name: str, args: dict) -> str:
        """Диспетчер вызовов инструментов плеера."""
        if func_name != 'play_media':
            return json.dumps({'error': f'Неизвестный инструмент: {func_name}'}, ensure_ascii=False)

        title = (args.get('title') or '').strip()
        if not title:
            return json.dumps({'error': 'Не указано название'}, ensure_ascii=False)

        # Поиск записи в media.db
        try:
            db = MediaDatabase(_db_path)
            records = db.export_all()
        except Exception as e:
            return json.dumps({'error': f'Ошибка базы данных: {e}'}, ensure_ascii=False)

        film_path = None
        film_title_found = title
        tl = title.lower()
        ...
        for rec in records:
            ...
            if (r_t == tl or r_ru == tl or r_orig == tl
                    or r_t.startswith(tl) or (r_ru and r_ru.startswith(tl))
                    or tl in r_t or tl in r_ru):
                if rec.get('path'):
                    film_path = rec['path']
                    film_title_found = rec.get('title_ru') or rec.get('title') or title
                    break

        if not film_path:
            return json.dumps({
                'error': f'Фильм или сериал "{title}" не найден в медиатеке',
                'suggestion': 'Проверьте правильность названия или попробуйте поиск'
            }, ensure_ascii=False)

        # Отправка WebSocket-команды на плеер (fire-and-forget)
        if room_id:
            try:
                ...
                logger.error(f'[play_dispatcher] Ошибка WebSocket broadcast: {e}')
                return json.dumps({'error': f'Ошибка запуска плеера: {e}'}, ensure_ascii=False)

        is_online = film_path.startswith(('http://', 'https://'))
        source_type = 'онлайн-источник' if is_online else 'локальный файл'

        return json.dumps({
            'success': True,
            'title': film_title_found,
            'path': film_path,
            'source': source_type,
            'message': f'Воспроизведение "{film_title_found}" запущено ({source_type})',
        }, ensure_ascii=False)
