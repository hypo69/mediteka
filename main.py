...
from src.fastapi import init_auth_router, init_chat_router, init_control_router, init_media_router, init_qbt_router, init_tts_router, init_logs_router, init_keys_router, init_admin_router, init_agents_router
...
if __name__ == '__main__':
    ...
    port: int = int(getattr(server_cfg, "port", 3000))
    if not port:
    ...
    if use_ssl and cert_file.exists() and key_file.exists():
        ...
        logger.info(f'Запуск сервера https://{host}:{port} (SSL включен)')
    else:
        logger.warning('Запуск без HTTPS (SSL выключен или сертификаты не найдены)')
        logger.info(f'Запуск сервера http://{host}:{port}')

    logger.info(f"Uvicorn autoreload: {'ВКЛЮЧЕН' if reload else 'ВЫКЛЮЧЕН'}")
