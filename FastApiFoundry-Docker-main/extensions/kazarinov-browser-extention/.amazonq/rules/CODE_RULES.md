...
Этот документ определяет **единые стандарты кодирования и документирования** для всех модулей проекта **AI Assistant (ai_assist)**.
...
Любая модель ИИ, использующаяся для работы с кодом проекта **AI Assistant (ai_assist)**, обязана:

1. **Прочитать файл `.amazonq/rules/DEPLOYMENT-WORKFLOW.md` полностью.**  
   В нём описано:
...
2. **Прочитать актуальные правила кодирования:**
   - `.amazonq/rules/CODE_RULES.md` (этот файл);
   - `.amazonq/rules/memory-bank/guidelines.md` — детальные паттерны кода.

3. **Прочитать базу знаний проекта:**
   - `.amazonq/rules/memory-bank/knowledge-base.md` — API, конфигурация, структура;
   - `.amazonq/rules/memory-bank/structure.md` — структура директорий.

4. **Следовать этим правилам дословно.**  
   Если модель не уверена — она должна явно указать на сомнение или запросить уточнение.
...
| Тип файла | Комментарии | Docstrings / JSDoc | Заголовок файла (header block) |
|---|---|---|---|
| `.js`, `.ts`, `.jsx`, `.tsx` | **English only** | **English only** | **English only** |
| `.css`, `.scss` | **English only** | **English only** | **English only** |
| `.html` | **English only** | — | **English only** |
| `.php` | **English only** | **English only** | **English only** |
| `.py`, `.ps1` | Russian or English | Russian or English | Russian or English |

> Независимо от того, на каком языке ведётся разработка и общение — JS/TS/CSS/HTML файлы **всегда** пишутся с комментариями на английском.
...
// ❌ Wrong — Russian in JS/TS/CSS/HTML is forbidden
...
**Версия:** 0.7.1 (AI Assistant Edition)
