...
const LocatorList: React.FC<LocatorListProps> = ({ locators, selectedLocatorKey, onSelect, onDelete }) => {
    return (
        <nav className="flex-1 overflow-y-auto">
            <h2 className="px-4 pt-2 text-lg font-semibold text-teal-400 sticky top-0 bg-gray-900">Locators</h2>
            <ul>
                    ...
                    <li key={key} className="px-2 py-1">
                        ...
                        >
                            <div className="flex items-center gap-2 truncate">
                               {selectedLocatorKey === key && <ChevronRightIcon className="w-5 h-5 text-teal-400 flex-shrink-0"/>}
                                <span className={`font-medium truncate ${selectedLocatorKey !== key ? 'ml-7' : ''}`}>{key}</span>
                            </div>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                ...
                                className="p-1 rounded-full hover:bg-red-500/20 text-gray-500 hover:text-red-400 transition-colors flex-shrink-0"
                                title={`Delete ${key}`}
                            >
                                <TrashIcon />
                            </button>
