...
export const PROVIDERS = {
    ...
    // opts.customUrl — base URL, e.g. http://localhost:9696/api/v1
    custom: {
        label: 'Custom (OpenAI-compatible)',
        placeholder: 'API key…',
        hint: '',
        fetchModels: async (apiKey, opts = {}) => {
            const base = (opts.customUrl || 'http://localhost:9696/api/v1').replace(/\/$/, '');
    ...
    // ── AI Assistant (FastAPI Foundry) ───────────────────────────────────────
    // Local AI orchestrator supporting multiple backends via model prefixes:
    ...
    // ollama::model-name → Ollama (local service)
    // No API key required — FastAPI Foundry is local and open by default.
    ai_assist: {
        label: 'AI Assistant (FastAPI Foundry)',
        placeholder: 'Leave empty (local)',
        hint: 'localhost:9696',
        fetchModels: async (apiKey, opts = {}) => {
            const base = (opts.customUrl || 'http://localhost:9696/api/v1').replace(/\/$/, '');
            const headers = apiKey ? { Authorization: `Bearer ${apiKey}` } : {};
