...
$StartScript = Join-Path $Root "Start-Project.ps1"

# ПОЧЕМУ ИСПОЛЬЗУЕТСЯ ДИНАМИЧЕСКАЯ ЗАГРУЗКА ФУНКЦИЙ:
#   Прямой вызов (dot-sourcing) файла Start-Project.ps1 приведет к немедленному запуску
...
if ($parseErrors.Count -gt 0) {
    throw "Start-Project.ps1 parse errors: $($parseErrors.Message -join '; ')"
}
